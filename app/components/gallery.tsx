    ...
    <>
      <section className="py-16 px-4 bg-gradient-to-b from-black/60 to-black/40 backdrop-blur-sm">
        <div className="container mx-auto">
          <h2 className="text-6xl font-[var(--font-cinzel)] mb-8 text-red-600 text-center">
            Qu'est ce que Evala ?
          </h2>
      ...
      <section className="py-16 px-4 bg-black/40 backdrop-blur-sm">
        <div className="container mx-auto">
          <h2 className="text-4xl font-bebas mb-12 text-red-600 text-center tracking-wider">
            Galerie Photo
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {galleryImages.map((image) => (
              <div
                key={image.id}
                className="relative group overflow-visible rounded-2xl shadow-2xl cursor-pointer transform transition-all duration-500 hover:-translate-y-2 hover:shadow-red-500/20"
              ...
              >
                ...
                <div className="aspect-w-16 aspect-h-9 relative h-[300px] bg-gradient-to-br from-black/5 to-black/20 rounded-2xl p-1">
                  <div className="relative w-full h-full rounded-xl overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500">
                      <div className="absolute bottom-0 left-0 right-0 p-6 transform translate-y-6 group-hover:translate-y-0 transition-transform duration-500">
                        <h3 className="text-white font-teko text-2xl mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">{image.alt}</h3>
                        <p className="text-gray-200 text-sm font-montserrat opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-200">{image.description}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {selectedImage && (
            ...
            >
              <div className="relative max-w-6xl w-full h-full flex items-center justify-center">
                <button
                  className="absolute top-4 right-4 text-white text-4xl hover:text-red-600 transition-colors z-10 bg-black/20 w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-sm"
                  onClick={() => setSelectedImage(null)}
                >
                  ×
                </button>
                <div className="relative w-full h-full max-h-[80vh] transform transition-all duration-500">
                  <Image
                    src={selectedImage.src}
                    alt={selectedImage.alt}
                    fill
                    className="object-contain transform transition-all duration-700 hover:scale-105"
                  />
                  <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 to-transparent">
                    <div className="max-w-3xl mx-auto text-center">
                      <h3 className="text-white font-teko text-3xl mb-3">{selectedImage.alt}</h3>
                      <p className="text-gray-200 font-montserrat text-lg">{selectedImage.description}</p>
                    </div>
                  </div>
                </div>
              </div>
