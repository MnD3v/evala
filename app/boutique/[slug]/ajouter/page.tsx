...
export default function AjouterProduitPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: b } = await supabase.from("boutiques").select("id, user_id").eq("slug", slug).single();
      if (!b || b.user_id !== session.user.id) { router.replace("/boutique"); return; }
      setBoutiqueId(b.id);
      setAuthChecked(true);
    };
    load();
  ...
  const handleSubmit = async () => {
    if (!form.nom.trim() || !boutiqueId || !userId) return;
  ...
  if (isSuccess) return (
    <PageShell>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div>
          <h2 className="font-clash font-bold text-black text-2xl mb-2">Produit publié !</h2>
          <p className="text-black/50 text-sm leading-relaxed">Votre produit est maintenant visible dans votre boutique.</p>
        </div>
    ...
    <PageShell>
      <div className="max-w-xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <h1 className="font-clash font-bold text-black text-3xl md:text-4xl mb-2">Ajouter un produit</h1>
          <p className="text-black/40 text-sm">Publiez une nouvelle création dans votre boutique.</p>
        </motion.div>
        ...
        <div className="bg-white border border-black/[0.07] rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.05)] overflow-hidden">
          <div className="p-6 md:p-8 space-y-6">
            ...
            <div>
              ...
              <input className={INPUT} value={form.nom} onChange={e => set("nom", e.target.value)} placeholder="Ex : Pagne kente, Collier en perles…" />
            </div>
            ...
            <div>
              ...
              <input className={INPUT} type="number" value={form.prix} onChange={e => set("prix", e.target.value)} placeholder="Ex : 5 000" min={0} />
            </div>
          ...
          <div className="flex justify-end px-6 md:px-8 py-4 border-t border-black/[0.06]">
            <button
              ...
              style={{ background: "#006A4E" }}
            >
              {isSubmitting ? <><Loader2 className="w-4 h-4 animate-spin" /> Publication…</> : <>Publier le produit <ArrowRight className="w-4 h-4" /></>}
            </button>
          </div>
        </div>
      </div>
